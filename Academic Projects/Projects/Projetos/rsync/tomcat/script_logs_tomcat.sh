...
source script_logs_tomcat.conf

#verificando se a chave existe
ls ${chave} &>/dev/null || echo -e "Chave ${chave} não existente \nTransferencia de logs não realizada \nVerificar em /home/centos/script_logs_tomcat.conf"
ls ${chave} &>/dev/null || exit 1

#liberando a chain INPUT do iptables para fazer o ssh
...
if [ ! -d ${PATH_logs} ]; then
        echo -e "Diretório ${PATH_logs} não existe em serviço local \nTransferencia de logs não realizada \nVerificar em /home/centos/script_logs_tomcat.conf"
        exit 1
fi
...
#Restaurando a regra padrão ipbtales para INPUT como DROP
